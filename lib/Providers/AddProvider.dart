...
import 'MainProviders.dart';
class AddProvider extends StatefulWidget {
  var Provider_max;
  AddProvider({this.Provider_max});
  @override
  AddProviderState createState() => AddProviderState();
}
...
class AddProviderState extends State<AddProvider> {
  String imagename;
  File sampleimage;
  var currentdate;
  int state = 0;
  ...
  TextEditingController contProviderid = new TextEditingController();
  TextEditingController contProvidername = new TextEditingController();
  TextEditingController contProviderremark = new TextEditingController();
  TextEditingController contProviderdesc = new TextEditingController();
  TextEditingController contProviderurl = new TextEditingController();
  TextEditingController contProviderdentrydate = new TextEditingController();
  getData() async {
  ...
  printlist(){
    if (cars != null) {
      ...
      {//print(cars.docs[i].data()['cat_id']);
        print('inside printlist cat${cars.docs[i].data()['Provider_id']}');
...
print('max=');
        print(Provider_max);
          ...
          contProviderid.text = Provider_max.toString();
                    ...
                    child: new Center(
                        child: Column(
                          children: <Widget>[
                            TextFormField(
                             // enabled: false,
                              controller: contProviderid,
                              validator: (input) {
                                if (input.isEmpty) {
                                  return 'Please provider Id ';
                                }
                              },
                        ...
                        TextFormField(
                      controller: contProviderdesc,
                      validator: (input) {
                        if (input.isEmpty) {
                          return 'Please provider Desc ';
                        }
                      },
                      onSaved: (input) => imagename = input,
                      decoration: InputDecoration(labelText: 'provider Desc'),
                      // obscureText: true,
                    ),


                            TextFormField(
                              controller: contProviderremark,
                              validator: (input) {
                                if (input.isEmpty) {
                                  return 'Please provider remark ';
                                }
                              },
                              onSaved: (input) => imagename = input,
                              decoration: InputDecoration(labelText: 'provider remark'),
                              // obscureText: true,
                            ),
...
///////
  ...
  Future<String> uploadimage() async {
    ...
    FirebaseStorage.instance.ref().child('${contProvidername.text}.jpg');
    ...
    addimagedata();
    contProviderid.text=(int.parse(contProviderid.text)+1).toString();
    contProvidername.clear();
    contProviderdesc.clear();
    return "";
  ...
  addimagedata() {
    ...
    FirebaseFirestore.instance.collection("Providers").doc().set({
      'Provider_id': contProviderid.text,
      'Provider_name': contProvidername.text,
      'Provider_desc': contProviderdesc.text,
      'Provider_remark':contProviderremark.text,
      'Provider_date':currentdate,// contProviderenterydate.text,
      'Provider_img': url2 == null ? 'no image': url2
    });
