...
class BookingSumBy extends StatefulWidget {
  @override
  _BookingSumByState createState() => _BookingSumByState();
}
...
class _BookingSumByState extends State<BookingSumBy> with TickerProviderStateMixin {
  @override
                ...
                child: GridView.builder(
                    ...
                    itemBuilder: (BuildContext context,int index) {
                      ...
                      return _build_summary(
                        ...
                        Booking_E: Bookinggroup[index]['Booking_E'],
                        Booking_W: Bookinggroup[index]['Booking_W'],
                ...
                child:GridView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      return _build_summary_yearly(
                        ...
                        Booking_E_yearly:Bookinggroupyearly[index]['Booking_E'] ,
                        Booking_W_yearly:Bookinggroupyearly[index]['Booking_W'] ,
                      );
                    }),
              ),
            ),
     ...
     // print( count);
      if (instlist.length > 0) {
        ...
        setState(() {
          ...
          for (int c = 0; c < Booking_date.length; c++) {
            ...
            for (int j = 0; j < instlist.length; j++) {
              if(instlist[j].booking_by=='Emad' &&
                  formatDate(
                      instlist[j].booking_startTime, [yyyy, '-', M,]) == Booking_date[c])
                {
                  setState(() {
                    temp_emad=temp_emad+1.0;
                  });

                }
              else if(instlist[j].booking_by=='Walid' &&
                  formatDate(
                      instlist[j].booking_startTime, [yyyy, '-', M,]) == Booking_date[c])

                {
                  setState(() {
                    temp_walid=temp_walid+1.0;
                  });
        ...
        for (int c = 0; c < Booking_year.length; c++) {
          ...
          _name_year=null;
          double temp_emad_yearly=0;
          double temp_walid_yearly=0;
          for (int j = 0; j < instlist.length; j++) {
            ...
            {
              setState(() {
                temp_emad_yearly=temp_emad_yearly+1.0;
              });

            }
            else if(instlist[j].booking_by=='Walid' &&formatDate(
                instlist[j].booking_startTime, [yyyy]) ==
                Booking_year[c])

            {
              setState(() {
                temp_walid_yearly=temp_walid_yearly+1.0;
              });
          ...
          setState(() {
            if (yearAmt > 0) {
              ...
              "Booking_E":temp_emad_yearly,
              "Booking_W":temp_walid_yearly
              });
            }
          });
...
class _build_summary extends StatelessWidget {
  ...
  final double Booking_E;
  final double Booking_W;
  _build_summary({
    this.Booking_date,
    // this.Shacks_from,
    this.Booking_total, this.Booking_E, this.Booking_W,
  });
  @override
  Widget build(BuildContext context) {
    return Card(
...
class _build_summary_yearly extends StatelessWidget {
  ...
  //final String Shacks_from;
  final double Booking_total;
  final double Booking_E_yearly;
  final double Booking_W_yearly;

  _build_summary_yearly({
    this.Booking_year,
    // this.Shacks_from,
    this.Booking_total, this.Booking_E_yearly, this.Booking_W_yearly,
  });
  @override
  Widget build(BuildContext context) {
    return Card(
