...
class MainShacksSummary extends StatefulWidget {

  @override
  _MainShacksSummaryState createState() => _MainShacksSummaryState();
}

class _MainShacksSummaryState extends State<MainShacksSummary> {
...
//              (a, b) => DateTime.parse(a['Shacks_date'] ['yyyy-M']).compareTo(DateTime.parse(b['Shacks_date'] ['yyyy-M'])));
      ...
      if(Shackslist.length>0) {
       ...
       Shacksfrom.add( Shackslist[i].Shack_owner_name);
          ...
          for (int i = 0; i < Shacksfrom.length; i++) {
            // print(Shackssfrom[i]);

            for (int c = 0; c < Shacks_date.length; c++) {
              temp_amt_do = 0.0;
              // print('rrrrr=${ShackssCat[c]}');
              for (int j = 0; j < Shackslist.length; j++) {

                if (Shackslist[j].Shack_owner_name == Shacksfrom[i]
                    && formatDate(Shackslist[j].Shack_come_date,[yyyy,'-',M,]) ==Shacks_date[c])
                {

                  setState(() {
                    var temp=Shackslist[j].Shack_amt>0 ?Shackslist[j].Shack_amt :0;
              ...
              print('Shacks_date[c]==${Shacks_date[c]}');
              print('Shacksfrom[i]==${Shacksfrom[i]}');

              print('temp_amt_do==${temp_amt_do}');
              setState(() {
                if (temp_amt_do>0)
                {
                  Shacksgroup.add({
                    "Shacks_date": Shacks_date[c],
                    "Shacks_from": Shacksfrom[i],
...
//                                            new MaterialPageRoute(
//                                                builder: (BuildContext context) =>
//                                                new MainShacksSummaryHistoryOne(
